...
/**
 * Helper genérico para exportar PDF con autoTable y footer simple.
 * options: { orientation, headStylesColor, fontSize, cellPadding }
 * footerLines: array de strings que se imprimirán debajo de la tabla (cada línea separada)
 */
const exportToPDF = ({
  title,
  head,
  ...
  const startY = configurarPDF(doc, title);
...
const exportToExcel = async ({ sheetName, title, headers, rows, summaryLines = [], columnWidths = [] }) => {
  ...
  if (summaryLines && summaryLines.length) {
    ...
    resumenRow.getCell(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF27AE60' } };
    resumenRow.getCell(1).alignment = { vertical: 'middle', horizontal: 'center' };
    resumenRow.height = 25;

    summaryLines.forEach(([label, value]) => {
      rowIndex++;
      worksheet.getRow(rowIndex).values = [label, value];
...
export const exportarCategoriasAPDF = (categorias) => {
  ...
  exportToPDF({
    ...
    body: tableData,
    footerLines,
    options: { headStylesColor: [41, 128, 185], fontSize: 9, cellPadding: 3 },
  });
};
...
export const exportarSubcategoriasAPDF = (subcategorias, categorias) => {
  ...
  const footerLines = [
    ...
    `Activas: ${subcategorias.filter(s => s.activo).length}`,
  ];

  exportToPDF({
    title: 'REPORTE DE SUBCATEGORÍAS',
    head: ['ID', 'Nombre', 'Categoría', 'Descripción', 'Estado'],
    body: tableData,
    footerLines,
    options: { headStylesColor: [52, 152, 219], fontSize: 8, cellPadding: 2 },
  });
};
...
export const exportarProductosAPDF = (productos) => {
  ...
  const footerLines = [
    ...
    `Stock total: ${productos.reduce((sum, p) => sum + p.stock, 0)} unidades`,
    `Valor inventario: $${valorTotal.toLocaleString('es-CO')}`,
  ];
  ...
  exportToPDF({
    ...
    body: tableData,
    footerLines,
    options: { orientation: 'landscape', headStylesColor: [46, 204, 113], fontSize: 8, cellPadding: 2 },
  });
};
...
export const exportarUsuariosAPDF = (usuarios) => {
  ...
  exportToPDF({
    ...
    body: tableData,
    footerLines,
    options: { headStylesColor: [241, 196, 15], fontSize: 8, cellPadding: 2 },
  });
};
...
export const exportarPedidosAPDF = (pedidos) => {
  ...
  exportToPDF({
    ...
    body: tableData,
    footerLines,
    options: { orientation: 'landscape', headStylesColor: [155, 89, 182], fontSize: 7, cellPadding: 2 },
  });
};
...
export const exportarCategoriasAExcel = async (categorias) => {
  ...
  const columnWidths = [8, 30, 50, 15, 18];
...
export const exportarSubcategoriasAExcel = async (subcategorias, categorias) => {
  const headers = ['ID', 'Nombre', 'Categoría', 'Descripción', 'Estado', 'Fecha Creación'];
  const rows = subcategorias.map(sub => {
    const categoria = categorias.find(c => c.id === sub.categoriaId);
    return [
      sub.id,
      sub.nombre,
  ...
  const columnWidths = [8, 30, 30, 50, 15, 18];

  await exportToExcel({
    sheetName: 'Subcategorías',
    title: 'REPORTE DE SUBCATEGORÍAS',
    headers,
    rows,
    summaryLines,
