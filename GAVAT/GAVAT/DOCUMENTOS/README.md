...
### Carrito
- `GET /api/cliente/carrito` - Ver carrito actual
- `POST /api/cliente/carrito` - Agregar producto
- `PUT /api/cliente/carrito/:id` - Actualizar cantidad
- `DELETE /api/cliente/carrito/:id` - Eliminar del carrito

### Pedidos
- `POST /api/cliente/pedidos` - Crear pedido (checkout)
- `GET /api/cliente/pedidos` - Mis pedidos (Cliente) / Todos (Admin)
- `GET /api/cliente/pedidos/:id` - Ver detalle
- `GET /api/admin/pedidos` - Ver todos los pedidos (Admin)
- `PUT /api/admin/pedidos/:id/estado` - Actualizar estado (Admin)

### Comentarios y Reseñas ⭐ NUEVO
- `POST /api/cliente/comentarios` - Crear comentario (Cliente autenticado)
- `GET /api/catalogo/productos/:productoId/comentarios` - Ver comentarios de producto
- `GET /api/admin/comentarios/pendientes` - Ver comentarios pendientes (Admin)
- `PUT /api/admin/comentarios/:id/moderar` - Aprobar/rechazar comentario (Admin)
- `DELETE /api/admin/comentarios/:id` - Eliminar comentario (Admin)
...
#### 📘 APIs y Pruebas
- **[MANUAL_APIS.md](backend/MANUAL_APIS.md)** - Documentación completa de APIs (Abril 2026)
  - Configuración inicial y variables de entorno
  - Todos los endpoints con ejemplos de request/response
  - Rutas públicas (Catálogo)
  - Rutas de cliente (Carrito, Pedidos, Comentarios)
  - Rutas de administrador (Gestión completa)
  - Códigos de respuesta HTTP
  - Flujos prácticos completos

- **[POSTMAN_COLLECTION.json](backend/POSTMAN_COLLECTION.json)** - Colección importable en Postman
  - Todas las rutas con ejemplos pre-configurados
  - Variables de entorno (tokens, URLs)
  - Estructura organizada por módulos
  - Listo para importar: `Postman → Import → JSON file`
